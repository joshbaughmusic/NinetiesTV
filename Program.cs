...
ResultPrinter.Print("All 80's Genres", All80sGenres(shows));
ResultPrinter.Print("All Distinct Genres", AllDistinctGenres(shows));
...
{
    return shows
        .Where(s => s.StartYear >= 1990 && s.EndYear <= 1999).ToList();
}
...
static string AllNamesWithCommasPlsAnd(List<Show> shows)
{
    ...
    string first = String.Join(", ", shows
        .Take(showCount - 1)
        .Select(s => $"{s.Name}"));
...
static string All80sGenres(List<Show> shows)
{
    return String.Join(", ", shows
...
static List<string> AllDistinctGenres(List<Show> shows)
{
    return shows
        .SelectMany(s => s.Genres)
        .Distinct()
...
static void NumShowsByYear(List<Show> shows)
{
    var filteredShows = shows
        .Where(s => s.StartYear >= 1987 && s.StartYear <= 2018)
        .GroupBy(s => s.StartYear);

    foreach (IGrouping<int, Show> group in filteredShows)
    {
        Console.WriteLine($"{group.Key}: {group.Count}");
    }
}

NumShowsByYear(shows);
